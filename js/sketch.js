...
var vecs = ['global cash card', 'pay card program', 'wisely pay card', 'adp payroll clients', 'credit card', 'pay card', 'financial wellness', 'early pay', 'pay active', 'payroll card', 'daily pay', 'debit card', 'direct deposit'];
preload();
...
function init() {
  ...
  for (let j = 0; j < vecs.length; j++) {
    if ( words_array.includes(vecs[j]) ) {
      let index = words_array.indexOf(vecs[j]);
